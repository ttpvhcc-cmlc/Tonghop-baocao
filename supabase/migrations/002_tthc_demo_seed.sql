...
-- TẬP TIN 002: DỮ LIỆU MẪU & BỘ KIỂM THỬ TỰ ĐỘNG (SEED & TEST SUITE v8 FINAL)
-- PHIÊN BẢN: 8.0 ENTERPRISE PRODUCTION READY
-- ĐẶC TÍNH: 100% IDEMPOTENT / STRICT ERROR ASSERTIONS / NON-MUTATING DEMO TEST SUITE
-- ==============================================================================
...
BEGIN
  RAISE NOTICE '======================================================================';
  RAISE NOTICE 'KHỞI TẠO DỮ LIỆU MẪU CHUẨN IDEMPOTENT SEED DATA (v8 FINAL)';
  RAISE NOTICE '======================================================================';
  ...
  -- ----------------------------------------------------------------------------
  -- SEED 3: BẢNG INDICATOR DEFINITIONS (3 CHỈ SỐ DO LƯỜNG NGHIỆP VỤ)
  -- ----------------------------------------------------------------------------
  INSERT INTO public.indicator_definitions (id, code, name, formula_key, unit_measure, target_value, description, display_order, active) VALUES
    (v_ind_online, 'ONLINE_RATE', 'Tỷ lệ Hồ sơ Trực tuyến', 'received_online / received_total * 100', '%', 80.00, 'Tỷ lệ hồ sơ được nộp qua cổng dịch vụ công trực tuyến', 1, true),
    (v_ind_ontime, 'ONTIME_RATE', 'Tỷ lệ Giải quyết Đúng hạn', '(completed_early + completed_on_time) / completed_total * 100', '%', 95.00, 'Tỷ lệ hồ sơ giải quyết trước hạn và đúng hạn', 2, true),
    (v_ind_overdue, 'OVERDUE_RATE', 'Tỷ lệ Hồ sơ Quá hạn', '(completed_late + pending_late) / received_total * 100', '%', 3.00, 'Tỷ lệ hồ sơ quá hạn trong quá trình xử lý', 3, true)
  ON CONFLICT (code) DO UPDATE SET
    name = EXCLUDED.name,
  ...
  -- ----------------------------------------------------------------------------
  -- SEED 4: CƠ CHẾ RESET SEED AN TOÀN - KHÔNG VI PHẠM TÍNH BẤT BIẾN LỊCH SỬ
  -- ----------------------------------------------------------------------------
  SELECT EXISTS(SELECT 1 FROM public.reports WHERE id = v_report_id) INTO v_report_exists;

  IF NOT v_report_exists THEN
    -- Khi chưa có báo cáo demo, tiến hành tạo mới báo cáo và quy trình chuyển đổi
    ...
    -- NARRATIVE PHÂN TÍCH KHỚP 100% SỐ LIỆU CSDL THỰC TẾ
    v_narrative_text := format(
      ...
      'Tỷ lệ nộp hồ sơ trực tuyến đạt %s%% (vượt chỉ tiêu 80.00%%). ' ||
      'Công tác giải quyết hồ sơ ghi nhận kết quả tích cực với Tỷ lệ giải quyết đúng và trước hạn đạt %s%% (vượt chỉ tiêu 95.00%%). ' ||
      'Tỷ lệ hồ sơ quá hạn được kiểm soát ở mức thấp %s%% (đạt yêu cầu dưới 3.00%%). ' ||
...
-- ==============================================================================
-- BỘ KIỂM THỬ TỰ ĐỘNG KHẮC KHẮT 20/20 (FINAL REAL TEST SUITE v8)
...
BEGIN
  RAISE NOTICE '======================================================================';
  RAISE NOTICE 'BẮT ĐẦU CHẠY BỘ KIỂM THỬ TỰ ĐỘNG 20/20 (FINAL REAL TEST SUITE v8)';
  RAISE NOTICE '======================================================================';
  ...
  INSERT INTO public.report_indicators (
    report_id, indicator_definition_id, scope_type, scope_id, calculated_value, formatted_value
  ) VALUES (
    v_suite_rep_id, v_ind_online, 'global', NULL, 100.0000, '100.00%'
  );
  ...
  IF v_snap_json->'units_snapshot' IS NULL OR v_snap_json->'fields_snapshot' IS NULL THEN
    RAISE EXCEPTION 'TEST 12 FAILED: Bản chụp snapshot thiếu thông tin tái tạo lịch sử units_snapshot hoặc fields_snapshot.';
  END IF;

  v_passed_tests := v_passed_tests + 1;
  RAISE NOTICE 'PASS Test 12: Tự động đóng băng bản chụp snapshot chứa đầy đủ thông tin tái tạo lịch sử.';
  ...
  v_test_failed := false;
  BEGIN
    DELETE FROM public.audit_logs WHERE entity_id = v_demo_report_id::text;
    v_test_failed := true;
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_sqlstate = RETURNED_SQLSTATE, v_sqlerrm = MESSAGE_TEXT;
    IF v_sqlerrm LIKE '%TEST % FAILED%' THEN RAISE EXCEPTION '%', v_sqlerrm; END IF;
  ...
  END IF;
  v_passed_tests := v_passed_tests + 1;
  RAISE NOTICE 'PASS Test 14: Nhật ký kiểm toán audit_logs là append-only tuyệt đối.';

  -- TEST 15: Client/authenticated cannot call write_audit_log (Permission Denied)
  v_test_failed := false;
  BEGIN
    PERFORM public.write_audit_log('UNAUTHORIZED_CLIENT_CALL', 'report', v_demo_report_id::text);
    v_test_failed := true;
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_sqlstate = RETURNED_SQLSTATE, v_sqlerrm = MESSAGE_TEXT;
    IF v_sqlerrm LIKE '%TEST % FAILED%' THEN RAISE EXCEPTION '%', v_sqlerrm; END IF;
    IF v_sqlstate <> '42501' AND POSITION('permission denied' IN v_sqlerrm) = 0 THEN
      RAISE EXCEPTION 'TEST 15 FAILED: Lỗi không xuất phát từ từ chối quyền gọi RPC write_audit_log (SQLSTATE 42501). SQLSTATE: %, Message: %', v_sqlstate, v_sqlerrm;
    END IF;
  END;
  IF v_test_failed THEN
    RAISE EXCEPTION 'TEST 15 FAILED: Client authenticated có thể tự do thực thi write_audit_log.';
  END IF;
  v_passed_tests := v_passed_tests + 1;
  RAISE NOTICE 'PASS Test 15: Chặn thành công quyền thực thi write_audit_log từ phía client authenticated.';

  -- TEST 16: Narrative generated_text matches database metrics 100%
  SELECT generated_text INTO v_analysis_text
  FROM public.report_analysis
  WHERE report_id = v_demo_report_id AND scope_type = 'overview'
  LIMIT 1;

  SELECT SUM(received_total) INTO v_sum_received
  FROM public.report_field_statistics
  WHERE report_id = v_demo_report_id;

  IF POSITION(v_sum_received::text IN v_analysis_text) = 0 THEN
    RAISE EXCEPTION 'TEST 16 FAILED: Văn bản nhận xét (%s) không chứa số liệu tổng tiếp nhận chính xác (%s).', v_analysis_text, v_sum_received;
  END IF;
  v_passed_tests := v_passed_tests + 1;
  RAISE NOTICE 'PASS Test 16: Nội dung phân tích nhận xét narrative khớp chính xác 100%% với số liệu CSDL.';
  ...
  -- TEST 18: Field to Unit mapping matches MASTER 100%
  ...
  -- Kiểm tra ràng buộc duy nhất chống trùng lặp (Anti-Double-Counting)
  v_test_failed := false;
  BEGIN
    INSERT INTO public.report_field_statistics (
      ...
      gen_random_uuid(), v_suite_rep_2_id, v_suite_src_id, v_field_1, 10, 10, 10, 10, 0, 0
    );
    v_test_failed := true;
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_sqlstate = RETURNED_SQLSTATE, v_sqlerrm = MESSAGE_TEXT;
    ...
    IF v_sqlstate <> '23505' AND POSITION('uq_report_source_field' IN v_sqlerrm) = 0 THEN
      RAISE EXCEPTION 'TEST 20 FAILED: Trùng lặp không bị chặn bởi uq_report_source_field. SQLSTATE: %, Message: %', v_sqlstate, v_sqlerrm;
    END IF;
  END;

  v_passed_tests := v_passed_tests + 1;
  RAISE NOTICE 'PASS Test 20: Quy tắc tích lũy đa nguồn và cơ chế chống trùng lặp dữ liệu chính xác tuyệt đối.';

  -- DỌN DẸP SẠCH SẼ CÁC BÁO CÁO THỬ NGHIỆM TẠM THỜI CỦA TEST SUITE
  -- (GIỮ NGUYÊN BÁO CÁO MẪU DEMO CHÍNH v_demo_report_id Ở TRẠNG THÁI LOCKED)
  DELETE FROM public.report_snapshots WHERE report_id IN (v_suite_rep_id, v_suite_rep_2_id);
  DELETE FROM public.report_analysis WHERE report_id IN (v_suite_rep_id, v_suite_rep_2_id);
  DELETE FROM public.report_indicators WHERE report_id IN (v_suite_rep_id, v_suite_rep_2_id);
  DELETE FROM public.report_field_statistics WHERE report_id IN (v_suite_rep_id, v_suite_rep_2_id);
  DELETE FROM public.report_sources WHERE report_id IN (v_suite_rep_id, v_suite_rep_2_id);
  DELETE FROM public.reports WHERE id IN (v_suite_rep_id, v_suite_rep_2_id);

  RAISE NOTICE '======================================================================';
  RAISE NOTICE 'KẾT QUẢ: TẤT CẢ %/20 BÀI KIỂM THỬ ĐÃ PASS THÀNH CÔNG RỰC RỠ!', v_passed_tests;
  RAISE NOTICE '======================================================================';
END;
$$;
