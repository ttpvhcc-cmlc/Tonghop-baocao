...
-- TẬP TIN 001: ĐỊNH NGHĨA CƠ SỞ DỮ LIỆU TOÀN DIỆN (SCHEMA v8 FINAL)
-- PHIÊN BẢN: 8.0 ENTERPRISE PRODUCTION READY
-- ĐẶC TÍNH: IDEMPOTENT / STRICT IMMUTABILITY / COMPOSITE FK / REVOKED AUDIT RPC
-- ==============================================================================
...
BEGIN
  ...
  -- 3. UPDATE: BẢO VỆ BẤT BIẾN TOÀN DIỆN KHI ĐÃ Ở LOCKED HOẶC ARCHIVED
  ...
  IF OLD.status = 'locked' AND NEW.status = 'archived' THEN
    IF OLD.report_code <> NEW.report_code OR
       ...
       COALESCE(OLD.locked_at, '-infinity'::timestamptz) <> COALESCE(NEW.locked_at, '-infinity'::timestamptz) THEN
  ...
  IF OLD.status <> NEW.status THEN
    ...
    IF NEW.status = 'locked' THEN
      ...
      -- SERVER BẮT BUỘC GHI NHẬN LOKED METADATA
      NEW.locked_at := now();
      ...
      -- ĐÓNG BĂNG BẢN CHỤP SNAPSHOT ĐẦY ĐỦ TRẠNG THÁI ĐỂ TÁI TẠO LỊCH SỬ (BAO GỒM FIELD->UNIT MAPPING)
